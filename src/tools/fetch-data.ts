(async () => {
  ...
  async function fetch_data(arguments) {
    ...
    const query = `FIND("${description.trim().toLowerCase()}", LOWER({Description}), 0)`;
    const URL_WITH_QUERY =
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Holidays?filterByFormula=` + encodeURIComponent(query);
  ...
  exports.fetch_data_config = config;
