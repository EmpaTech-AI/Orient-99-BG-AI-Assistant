(async () => {
  ...
  async function fetch_cruise(arguments) {
    ...
    const query = `FIND("${destination.trim().toLowerCase()}", LOWER({Destination}), 0)`;
    const URL_WITH_QUERY =
      `https://api.airtable.com/v0/${AIRTABLE_BASE_ID}/Cruise?filterByFormula=` + encodeURIComponent(query);
  ...
  exports.fetch_cruise_config = config;
  exports.fetch_cruise = fetch_cruise;
})();
